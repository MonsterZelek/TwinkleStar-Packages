...
if [ $? -eq 0 ]; then
	mv /usr/bin/set_default_page_to_TwinkleStar_portal.sh /usr/bin/switch_to_default_luci.sh
	mv /www/index.html /www/index.htm 
	mv /www/TwinkleStar_Portal.html /www/index.html
	echo 'Now we have set the default page to TwinkleStar Portal.'
	exit 0
fi
